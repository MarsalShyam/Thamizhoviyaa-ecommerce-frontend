            ...
            return (
              <React.Fragment key={step.id}>
                <button
                  ...
                  disabled={step.id > currentStep && !isEdit} // Allow jumping steps only if editing or completed
                ...
                >
                  ...
                  <div className="text-left">
                    <span className="text-xs text-gray-400 block font-medium">Step {step.id}</span>
                    <span className={`text-sm font-semibold ${
                      isActive ? 'text-primary-600' :
                      isCompleted ? 'text-green-600' : 'text-gray-500 group-hover:text-gray-700'
                    }`}>{step.name}</span>
                  </div>
                </button>
                {idx < STEPS.length - 1 && (
                  <div className="hidden md:block flex-grow h-0.5 bg-gray-100 mx-4" />
                )}
              </React.Fragment>
            );
          })}
              ...
              {currentStep === 4 && (
                <div className="space-y-6">
                  <div className="flex justify-between items-center border-b pb-2">
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">Custom Attributes</h3>
