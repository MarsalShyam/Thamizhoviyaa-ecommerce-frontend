...
const Checkout = () => {
  ...
  // Save newly entered address to user profile (same logic as AddressManagement)
  const saveNewAddressToDB = async (address) => {
  try {
    ...
    // Save to DB (same as AddressManagement)
  ...
  const placeOrderOnServer = async (paymentDetails = {}) => {
    ...
    if (!selectedAddress && validateAddress(newAddress)) {
      await saveNewAddressToDB(newAddress);
    }
